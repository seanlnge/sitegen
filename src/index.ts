...
export async function Build(igHandle: string, photoCount: number, chosenTemplate?: string) {
    ...
    const imgdesc = await messageChain.describeImages(images, "Describe this image in a paragraph, including the color scheme, the main item present in this image, the overall feeling that this image presents, as well as where on a website that this image might make sense.");
    ...
    const website = await messageChain.queryModel("gpt-4o", true);
...
export async function ReviseBuild(messageChain: MessageChain, template: TemplateBuilder, revisionMessage: string, firstRevise: boolean) {
    ...
    const reviseChain = firstRevise ? new MessageChain({ logPath: 'reviselog.txt', saveLog: true }) : messageChain;
